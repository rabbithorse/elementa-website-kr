<template>
  <div class="gnb lg:static ">
    <div class="gnb-overlay lg:hidden max-lg:fixed inset-0 bg-black/50 z-40" @click="handleOverlayClick"></div>
    <nav class="menu absolute w-[80vw] right-0 top-0 h-full z-50 bg-black/90 lg:bg-transparent lg:static lg:h-auto lg:w-auto lg:flex lg:items-center lg:justify-center lg:py-0 py-10 px-6 lg:px-0">
      <ul class="menu-list depth1-menu-list flex lg:flex-row flex-col 2xl:gap-x-12 xl:gap-x-8 ">
        <li v-for="item in menuItems" :key="item.name" :class="['relative', item.children && item.children.length ? 'group' : '']">
          <NuxtLink :to="item.path" class="menu-link depth1-menu-link flex items-center justify-center text-white 2xl:py-4 py-3 xl:px-7 px-5 text-base leading-[130%] rounded-md after:absolute after:content-[''] after:w-full after:h-full after:bg-white/0 after:transition after:duration-300 after:ease-in-out lg:hover:after:bg-white/[0.01] lg:group-hover:after:bg-white/[0.01]">{{ item.name }}</NuxtLink>

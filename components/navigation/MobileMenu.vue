          ...
          <div v-accordion="subMenuOpen">
            <ul v-if="item.children && item.children.length" class="depth2-menu-list px-4">
              <li v-for="child in item.children" :key="child.name">
                <NuxtLink :to="child.path" class="menu-link depth2-menu-link inline-block w-full text-white py-2 px-5 text-base leading-[130%] lg:hover:bg-black/10 transition text-left">{{ child.name }}</NuxtLink>
              </li>
            </ul>
          </div>
        </li>
